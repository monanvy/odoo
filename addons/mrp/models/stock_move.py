...
class StockMove(models.Model):
    ...
    def action_done(self):
        production_moves = self.filter_production_move()
        production_moves.move_validate()
        return super(StockMove, self-production_moves).action_done()

    def filter_production_move(self):
        production_moves = self.filtered(lambda move: (move.production_id or (move.raw_material_production_id and move.raw_material_production_id.state != 'done')) and not move.scrapped)
        return production_moves
    # End: Trobz change
