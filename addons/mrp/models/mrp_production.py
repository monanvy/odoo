...
#                 order.availability = (all(assigned_list) and 'assigned') or (any(partial_list) and 'partially_available') or 'waiting'
    ...
    @api.multi
    @api.depends('move_raw_ids.state', 'move_raw_ids.reserved_quant_ids',
                 'workorder_ids.move_raw_ids', 'bom_id.ready_to_produce')
    def _compute_availability(self):
        for order in self:
            ...
            if order.bom_id.ready_to_produce == 'all_available':
                ...
                assigned_list = [x.state in ('assigned', 'done', 'cancel')
                                 for x in order.move_raw_ids]
                if all(assigned_list):
                    order.availability = 'assigned'
                else:
                    availability = any(
                        [x.quantity_available < x.unit_factor
                         for x in order.move_raw_ids]
                        ) and 'waiting' or 'partially_available'
